...
hlm_mod_intercept =
  lme(fixed = receita ~ nps + tempo_medio_contrato + perc_fidelidade + cluster + inflacao + desemprego,
      random = ~ 1 | cidade,
      data = vendas,
      method = "REML")

#Estimação do modelo com Interceptos e Inclinações Aleatórios
hlm_mod_intercept_inclin =
  lme(fixed = receita ~ 
        #preditoras
        nps + tempo_medio_contrato + perc_fidelidade + cluster + inflacao + desemprego + inflacao + desemprego,
      random = ~ nps | cidade,
      data = vendas,
      method = "REML")
...
hlm_mod_final =
  lme(fixed = receita ~ 
        #preditoras
        nps + perc_fidelidade +  inflacao +
        #inclinações dos grupos
        nps:inflacao +
        perc_fidelidade:inflacao,
      random = ~ nps + perc_fidelidade + inflacao | cidade,
      data = vendas,
      method = "REML")
...
ols_mod_final <- 
  lm(formula = receita ~ nps + tempo_medio_contrato + perc_fidelidade + cluster + inflacao + desemprego,
     data = vendas)
